...
module.exports = function(require){
  ...
  out('~~[visual-stack] ~bc~See~w~ your code.');
  ...
  function processArgs(arg){
    ...
    function usage(err){
      out(
        '~br~'+err+'\n' +
        '~w~Usage:\n' +
        '~w~visual-stack ~c~[flag] ~g~target ~y~[args]\n' +
        ...
        '  ~c~-ui:port ~w~Set the UI port. default: 2000\n' +
  ...
  function uiSender(port, bind){
    ...
    ui.listen(port, bind)
    out("~~[visual-stack]~w~ WebGL trace UI: http://"+bind+":"+port)
  ...
  function browserJSMode(filter, port, bind, root, sender){
    ...
    out("~~[visual-stack]~w~ Serving browser JS: http://"+bind+":"+port)
  ...
  function proxyMode(filter, port, bind, proxy, sender){
    ...
    //appHttp.watcher = define.watcher()
    out("~~[visual-stack]~w~ Proxying browser JS: http://"+bind+":"+port+" -> "+proxy)
